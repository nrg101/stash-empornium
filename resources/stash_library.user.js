...
(function () {
    ...
    if (!TRACKERS.includes(location.hostname)) {
        const stash = function () {
            ...
            const reloadImg = url => fetch(url, {cache: 'reload', mode: 'no-cors'})
            ...
            class Stash extends EventTarget {
                ...
                async getVersion() {
                    const reqData = {
                        "operationName": "", "variables": {}, "query": `query version {
    version {
        version
    }
}
`
                ...
                async runPluginTask(pluginId, taskName, args = []) {
                    const reqData = {
                        ...
                        "variables": {
                            "plugin_id": pluginId, "task_name": taskName, "args": args
                        },
                ...
                async callGQL(reqData) {
                    const options = {
                        method: 'POST', body: JSON.stringify(reqData), headers: {
                ...
                async getPlugins() {
                    const reqData = {
                        "operationName": "Plugins", "variables": {}, "query": `query Plugins {
  plugins {
    id
    name
    description
...
`
                ...
                async getStashBoxes() {
                    const reqData = {
                        "operationName": "Configuration", "variables": {}, "query": `query Configuration {
                        configuration {
                          general {
                            stashBoxes {
                              endpoint
        ...
        <input id="${inputId}" class="bg-secondary text-white border-secondary form-control" placeholder="${placeholder}">
                ...
                async updateConfigValueTask(sectionKey, propName, value) {
                    return this.runPluginTask("userscript_functions", "Update Config Value", [{
                        "key": "section_key", "value": {"str": sectionKey}
                    }, {"key": "prop_name", "value": {"str": propName}}, {"key": "value", "value": {"str": value}}]);
                }

                async getConfigValueTask(sectionKey, propName) {
                    await this.runPluginTask("userscript_functions", "Get Config Value", [{
                        "key": "section_key", "value": {"str": sectionKey}
                    }, {"key": "prop_name", "value": {"str": propName}}]);
                ...
                parseSearchItem(searchItem) {
                    ...
                    return {
                        urlNode, url, id, data, nameNode, name, queryInput, performerNodes
                    }
                }
                ...
                parseSearchResultItem(searchResultItem) {
                    ...
                    for (const matchNode of matchNodes) {
                        ...
                        matches.push({
                            matchType, matchNode, entityNode, matchName, remoteName, data
                        });
                    }
